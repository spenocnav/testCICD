# ==============================================================================
# extract_altimetria.py — Extrae lecturas de altimetría (StatusData) a fact_status_readings
#
# Diagnostic ID: aZ_PCPTFQJUWGgwTodd5nhA  (INMUTABLE — plan §5.4)
# Chunk: 10 días
...
SCRIPT_ID        = 'altimetria'
DIAGNOSTIC_ID    = DIAGNOSTIC_IDS['altimetria']   # INMUTABLE
...
def extract_chunk(api, devices: list, from_str: str, to_str: str, db_name: str) -> pd.DataFrame:
    """
    Envía 1 multi-call con 1 request por device para el diagnostic de altimetría.
    Devuelve un DataFrame con las columnas de fact_status_readings.
    """
    requests_list = [
        ['Get', {
            'typeName': 'StatusData',
...
def main():
    run_status_extraction(
        ...
        chunk_days=CHUNK_SIZE_DAYS['altimetria'],
